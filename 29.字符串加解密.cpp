...
char decrypt[] = {"bcdefghijklmnopqrstuvwxyzaBCDEFGHIJKLMNOPQRSTUVWXYZA1234567890"};
void Encrypt(char aucPassword[],char aucResult[])
{
	...
	while(aucPassword[i] != '\0')
	{
		if(aucPassword[i] >= '0' && aucPassword[i] <= '9')
			aucResult[i] = decrypt[aucPassword[i] - '0' + 52];
		else if(aucPassword[i] >='A' && aucPassword[i] <= 'Z')
			aucResult[i] = decrypt[aucPassword[i] - 'A'];
		else if(aucPassword[i] >= 'a' && aucPassword[i] <= 'z')
			aucResult[i] = decrypt[aucPassword[i] - 'a' + 26];
		cout<< aucResult[i++];
	}
	cout<<endl;
	return;
